...
<div class="wrapper">
    ...
    <span class="title">管理者账号管理>修改</span><!-- End of title -->
		...
		<form action="${basePath}users.do" method="post" name="baseForm" />
			...
			<table width="634" class="tb2_green" border="0" cellspacing="0" cellpadding="0">
	          ...
	          <tr>
	            <th><span>*</span> 状态：</th>
	            <td bgcolor="#ececec">
	            	<input type="radio" name="status" value="1"<c:if test="${us.status  == 1}">checked</c:if>>启用</input>
	            	<input type="radio" name="status" value="0" <c:if test="${us.status == 0}">checked</c:if>>停用</input>
	            </td>
	          </tr>
